# =============================================================================
# Sensitivity analysis (overall stratum) -- additionally adjusted for parental
# education. Runs LW + GP sequentially. Adds pedu_cat to:
#   (1) outcome model formula (handled inside function_education_sen.R)
#   (2) joint distribution sampling (4-way: gender x raceth x educat x pedu_cat)
# Excludes person-years with missing pedu_cat.
# =============================================================================
...
# Exclude rows with missing covariates (including pedu_cat for parental analysis)
...
# 4-way joint sampling: (gender, raceth, educat, pedu_cat) tuples
gender <- raceth <- educat <- pedu_cat <- rep(NA_character_, sim_length)
...
for (i in 1:n_cohort){
  ...
  cohort_data <- APC_df[APC_df$cohort == cohort_levels[i],
                        c("gender", "raceth", "educat", "pedu_cat")]
...
sim1 <- as.data.frame(cbind(gender, raceth, educat, pedu_cat), stringsAsFactors = FALSE)
sim2 <- cbind(sim, sim1)
...
saveRDS(edu, paste0("/restricted/project/gly-ukb/Ruijia/Output/sen/parental/output_all_LW_", out_date, ".RDS"))
...
saveRDS(edu.nc2, paste0("/restricted/project/gly-ukb/Ruijia/Output/sen/parental/output_nc_all_LW_", out_date, ".RDS"))
saveRDS(edu.cf2, paste0("/restricted/project/gly-ukb/Ruijia/Output/sen/parental/output_cf_all_LW_", out_date, ".RDS"))
...
saveRDS(edu, paste0("/restricted/project/gly-ukb/Ruijia/Output/sen/parental/output_all_GP_", out_date, ".RDS"))
...
saveRDS(edu.nc2, paste0("/restricted/project/gly-ukb/Ruijia/Output/sen/parental/output_nc_all_GP_", out_date, ".RDS"))
saveRDS(edu.cf2, paste0("/restricted/project/gly-ukb/Ruijia/Output/sen/parental/output_cf_all_GP_", out_date, ".RDS"))
