# =============================================================================
# Main analysis (overall / "all" stratum):
#   Runs the cohort-difference decomposition using both dementia classification
#   algorithms as co-primary analyses, sequentially within the same script.
#   For each algorithm, the analytic sample, joint distribution sampling, and
#   simulation grid are constructed independently from that algorithm's sample.
#
# Outputs:
#   /Output/main/output_all_LW_<date>.RDS       (LW raw bootstrap matrix)
#   /Output/main/output_nc_all_LW_<date>.RDS    (LW collapsed nc)
#   /Output/main/output_cf_all_LW_<date>.RDS    (LW collapsed cf)
#   /Output/main/output_all_GP_<date>.RDS       (GP raw bootstrap matrix)
#   /Output/main/output_nc_all_GP_<date>.RDS    (GP collapsed nc)
#   /Output/main/output_cf_all_GP_<date>.RDS    (GP collapsed cf)
# =============================================================================
...
####keep needed columns; drop covariate missingness
...
###Number of cohorts, ages and periods
n_cohort<-length(unique(APC_df$cohort))
n_age<-length(unique(APC_df$age))
n_period<-length(unique(APC_df$period))

##Unique combinations * n observations
groups <- n_cohort*n_age*n_period
sim_length <- n_obs*n_cohort*n_age*n_period
sim_length_cohort <- n_obs*n_age*n_period

# Joint sampling (revised per R2): draw whole (gender, raceth, educat) tuples
# from each cohort's empirical data to preserve within-cohort covariate
# structure, instead of independent marginal draws via rbinom/rmultinom.
gender <- raceth <- educat <- rep(NA_character_, sim_length)
...
sim1 <- as.data.frame(cbind(gender, raceth, educat), stringsAsFactors = FALSE)

sim2 <- cbind(sim, sim1)

# Set factor levels to match APC_df (preserves model parameterization)
...
edu <- foreach(bs = 1:bs_it, .combine='cbind', .inorder=FALSE, .packages=c("cfdecomp","nnet","Epi","Hmisc","fabricatr")) %dopar% {
  mediation_analysis(outcome="langa_di", mediator="educat", dataframe=APC_df, sim.df=edu.sim, mc_iteration=mc_it, reference_cohort = "WB 1942-47") #default ref.cohort = 1945
}
...
saveRDS(edu, paste0("/restricted/project/gly-ukb/Ruijia/Output/main/output_all_LW_", out_date, ".RDS"))

edu.nc <- edu[1:(nrow(sim)),] #first half is nc, second half is cf
...
saveRDS(edu.nc2, paste0("/restricted/project/gly-ukb/Ruijia/Output/main/output_nc_all_LW_", out_date, ".RDS"))
saveRDS(edu.cf2, paste0("/restricted/project/gly-ukb/Ruijia/Output/main/output_cf_all_LW_", out_date, ".RDS"))


# =============================================================================
#  Gianattasio-Power analysis (co-primary)
# =============================================================================
...
saveRDS(edu, paste0("/restricted/project/gly-ukb/Ruijia/Output/main/output_all_GP_", out_date, ".RDS"))
...
saveRDS(edu.nc2, paste0("/restricted/project/gly-ukb/Ruijia/Output/main/output_nc_all_GP_", out_date, ".RDS"))
saveRDS(edu.cf2, paste0("/restricted/project/gly-ukb/Ruijia/Output/main/output_cf_all_GP_", out_date, ".RDS"))
